...
class Basemap(object):
        ...
        if self.resolution is not None and len(self.coastpolygons) > 0:
            ...
            for x,y,typ in zip(x,y,self.coastpolygontypes):
                ...
                if typ == 1:
                    ...
                    self.shapelylandpolygons.append(shapely.Polygon(b))
    ...
    def intersects_land(self,x1,y1,x2,y2):
        if self.resolution is None: return None
        intersects = False
        line = shapely.LineString([(x1,y1),(x2,y2)])
        for poly in self.shapelylandpolygons:
            intersects = poly.intersects(line)
            #print(intersects)
            #print(line)
            if intersects:
                break
    ...
    def is_sea(self,xpt,ypt):
        """
        Returns True if the given x,y point (in projection coordinates) is
        over sea, False otherwise.  The definition of land is based upon
        the GSHHS coastline polygons associated with the class instance.
        Points over lakes inside land regions are counted as land points.
        """
        if self.resolution is None: return None
        landpt = False
        for poly in self.landpolygons:
