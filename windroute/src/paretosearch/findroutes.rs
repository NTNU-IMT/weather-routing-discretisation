        ...
        for next_index in 0..graph[current].len() {
            for t_current in 0..time_steps {
                if cost[current][t_current].is_finite() {
                    if weather_twa[current][t_current].is_finite()
                        && weather_tws[current][t_current].is_finite()
                    {
                        ...
                        for mode_index in 0..modes_speed.len() {
                            ...
                            if duration.is_finite() {
                                let t_next: usize =
                                    (((time_resolution * (t_current as f64) + duration)
                                        / time_resolution)
                                        .round() as usize)
                                        .min(time_steps - 1);
                        ...
                        unimplemented!("Need weather twa and weather tws different form infinity");
                    }
                }
            }
        }
